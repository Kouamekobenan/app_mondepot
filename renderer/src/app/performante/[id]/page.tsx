...
const Page: React.FC = () => {
  ...
  // Fetch des données avec gestion d'erreur améliorée
  ...
  // Liste unique des livreurs avec validation
  ...
  // Calcul des données du graphique et statistiques
  const { chartData, monthlyStats } = useMemo((): {
    chartData: ChartDataPoint[];
    monthlyStats: MonthlyStats;
  } => {
    // Filtrage et validation des livraisons
    const filteredDeliveries = deliveries
      ...
      .filter((delivery) => {
        ...
        return (
          ...
          !isNaN(deliveryDate.getTime()) // Validation de date
        );
      });
    ...
    filteredDeliveries.forEach((delivery) => {
      ...
      // Calcul avec validation des données
    ...
    const chartData: ChartDataPoint[] = Object.entries(dailyTotals)
      ...
      .sort((a, b) => {
        ...
        const dateA = new Date(
          `2025-${selectedMonth + 1}-${a.date.split("/")[0]}`
        );
        const dateB = new Date(
          `2025-${selectedMonth + 1}-${b.date.split("/")[0]}`
        );
      ...
      <div className="flex-1 p-6">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold text-gray-800 mb-8 text-center">
            <span>Performance des livreurs en </span>
